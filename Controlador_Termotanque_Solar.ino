...
AltSoftSerial serialBlue;      // TX del HM-10 va a pin 8 y RX del HM-10 va a pin 9.
...
int estadoApp = 1;   // Estado de todo el sistema
//------------------------------------------------------------------
...
void loop()
{
  ...
  if (estadoApp == 1)
  { // Analiza que el sistema esté "ON" según instrucción del usuario
    if (t.hour > 5 && t.hour < 22)
    {
      activarResistencia();
    } // Analiza que se encuentre dentro de los parámetros horarios (esto despues hacerlo para que lo pueda modificar el usuario desde la App)
    else
    {
      digitalWrite(Rele, LOW);
    } // Fuera de horario resistencia apagada
  }
  else
  {
    digitalWrite(Rele, LOW);
  } // Si sistema OFF, resistencia apagada
  ...
  {
    estadoActual = "0";
  }               // Analiza si el relé está prendido o ensendido, quizás se puede hacer más simple para sacar el digitalRead
...
void escribirBlue()
{
  ...
  serialBlue.print(":");
  if (t.min < 10)
  { // Agrega un cero para que el tiempo siempre aparezca como HH:MM
    serialBlue.print("0");
...
void leerBlue()
{
  while (serialBlue.available()) // Analiza si hay data llegando desde el BLE, la lee y analiza.
  {
    ...
    case 48:
      estadoApp = 0;
      break; // Si lee 0 significa que el sistema pasa a estar OFF
    case 49:
      estadoApp = 1;
      break; // Si lee 1 significa que el sistema pasa a estar ON
    case 51:
      setPoint++;
      break; // Si lee 3 significa que el Set Point sube 1 grado
    case 52:
      setPoint--;
      break; // Si lee 4 significa que el Set Point baja 1 grado
    }
  }
}
void datosSensores()
...
void imprimirSerial()
{ // Actualmente sin uso, solo está para el prototipo
  Serial.println("----------------------------------------------------------------");
...
{ // Analiza la temperatura y actúa acorde. Averigüar si es mas eficiente cambiar a PID y como sería.
